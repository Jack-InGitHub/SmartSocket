...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagSocket1)
    {
        info->event[info->num] = EVENT_Socket1;
        info->num++;
        dataPoints->valueSocket1 = gizStandardDecompressionValue(Socket1_BYTEOFFSET,Socket1_BITOFFSET,Socket1_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }

    if(0x01 == issuedData->attrFlags.flagSocket2)
    {
        info->event[info->num] = EVENT_Socket2;
        info->num++;
        dataPoints->valueSocket2 = gizStandardDecompressionValue(Socket2_BYTEOFFSET,Socket2_BITOFFSET,Socket2_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }


    if(0x01 == issuedData->attrFlags.flagOnDay_1)
    {
        info->event[info->num] = EVENT_OnDay_1;
        info->num++;
        dataPoints->valueOnDay_1 = gizX2Y(OnDay_1_RATIO,  OnDay_1_ADDITION, issuedData->attrVals.valueOnDay_1);
    }

    if(0x01 == issuedData->attrFlags.flagOnHour_1)
    {
        info->event[info->num] = EVENT_OnHour_1;
        info->num++;
        dataPoints->valueOnHour_1 = gizX2Y(OnHour_1_RATIO,  OnHour_1_ADDITION, issuedData->attrVals.valueOnHour_1);
    }

    if(0x01 == issuedData->attrFlags.flagOnMinute_1)
    {
        info->event[info->num] = EVENT_OnMinute_1;
        info->num++;
        dataPoints->valueOnMinute_1 = gizX2Y(OnMinute_1_RATIO,  OnMinute_1_ADDITION, issuedData->attrVals.valueOnMinute_1);
    }

    if(0x01 == issuedData->attrFlags.flagOffDay_1)
    {
        info->event[info->num] = EVENT_OffDay_1;
        info->num++;
        dataPoints->valueOffDay_1 = gizX2Y(OffDay_1_RATIO,  OffDay_1_ADDITION, issuedData->attrVals.valueOffDay_1);
    }

    if(0x01 == issuedData->attrFlags.flagOffHour_1)
    {
        info->event[info->num] = EVENT_OffHour_1;
        info->num++;
        dataPoints->valueOffHour_1 = gizX2Y(OffHour_1_RATIO,  OffHour_1_ADDITION, issuedData->attrVals.valueOffHour_1);
    }

    if(0x01 == issuedData->attrFlags.flagOffMinute_1)
    {
        info->event[info->num] = EVENT_OffMinute_1;
        info->num++;
        dataPoints->valueOffMinute_1 = gizX2Y(OffMinute_1_RATIO,  OffMinute_1_ADDITION, issuedData->attrVals.valueOffMinute_1);
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueSocket1 != cur->valueSocket1)
    {
        GIZWITS_LOG("valueSocket1 Changed\n");
        ret = 1;
    }
    if(last->valueSocket2 != cur->valueSocket2)
    {
        GIZWITS_LOG("valueSocket2 Changed\n");
        ret = 1;
    }
    if(last->valueOnDay_1 != cur->valueOnDay_1)
    {
        GIZWITS_LOG("valueOnDay_1 Changed\n");
        ret = 1;
    }
    if(last->valueOnHour_1 != cur->valueOnHour_1)
    {
        GIZWITS_LOG("valueOnHour_1 Changed\n");
        ret = 1;
    }
    if(last->valueOnMinute_1 != cur->valueOnMinute_1)
    {
        GIZWITS_LOG("valueOnMinute_1 Changed\n");
        ret = 1;
    }
    if(last->valueOffDay_1 != cur->valueOffDay_1)
    {
        GIZWITS_LOG("valueOffDay_1 Changed\n");
        ret = 1;
    }
    if(last->valueOffHour_1 != cur->valueOffHour_1)
    {
        GIZWITS_LOG("valueOffHour_1 Changed\n");
        ret = 1;
    }
    if(last->valueOffMinute_1 != cur->valueOffMinute_1)
    {
        GIZWITS_LOG("valueOffMinute_1 Changed\n");
        ret = 1;
    }
    ...
    if(last->valueRH != cur->valueRH)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueRH Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints, devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Socket1_BYTEOFFSET,Socket1_BITOFFSET,Socket1_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSocket1);
    gizStandardCompressValue(Socket2_BYTEOFFSET,Socket2_BITOFFSET,Socket2_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSocket2);
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));

    devStatusPtr->valueOnDay_1 = gizY2X(OnDay_1_RATIO,  OnDay_1_ADDITION, dataPoints->valueOnDay_1);
    devStatusPtr->valueOnHour_1 = gizY2X(OnHour_1_RATIO,  OnHour_1_ADDITION, dataPoints->valueOnHour_1);
    devStatusPtr->valueOnMinute_1 = gizY2X(OnMinute_1_RATIO,  OnMinute_1_ADDITION, dataPoints->valueOnMinute_1);
    devStatusPtr->valueOffDay_1 = gizY2X(OffDay_1_RATIO,  OffDay_1_ADDITION, dataPoints->valueOffDay_1);
    devStatusPtr->valueOffHour_1 = gizY2X(OffHour_1_RATIO,  OffHour_1_ADDITION, dataPoints->valueOffHour_1);
    devStatusPtr->valueOffMinute_1 = gizY2X(OffMinute_1_RATIO,  OffMinute_1_ADDITION, dataPoints->valueOffMinute_1);

    devStatusPtr->valueCurrent = exchangeBytes(gizY2XFloat(Current_RATIO,  Current_ADDITION, dataPoints->valueCurrent));
    ...
    devStatusPtr->valueRH = exchangeBytes(gizY2XFloat(RH_RATIO,  RH_ADDITION, dataPoints->valueRH));
